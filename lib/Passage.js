...
class Passage {
  ...
  render (source) {
    ...
    /**
     * Create footnotes from Markdown
     * https://github.com/markedjs/marked/issues/1562#issuecomment-551269829
     *
     * @function createFootnotes
     * @private
     * @param {Array} markdown - a string of Markdown flavored text
     * @returns {string} A string of HTML
     */
    function createFootnotes (markdown) {
      const footnotes = [];
      const newTokens = [];
      const footnoteTest = /^\[\^[^\]]+\]: /;
      // const footnoteMatch = /^\[\^([^\]]+)\]: ([\s\S]*)$/;
      const footnoteMatch = /^\[\^([^\]]+)\]: (.+)$/gm;
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        const matches = tokens[i].text.matchAll(footnoteMatch);
